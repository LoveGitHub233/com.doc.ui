 ...
 * version: 1.7.3
 * number:002
 * 2019.08.16版本跟随 上海药品审评核查中心20190821 产品发送
 */
"use strict";
...
{	
  ...
  openWindow:function( strURL,IsShowOnExtendMonitor,ProductCaption, ntkoBrowserProductKey, ntkoBrowserNoExpireKey, ntkoBrowserOptions )
  {

    var testurl =window.location.search;
    var docUrl=document.location.toString();
    var wpathname= window.document.location.pathname;
    var pos=docUrl.indexOf("/#");
    ...
    if (window.top!=window.self) {
      ...
      var winend= winTopHref.length;
      var wint=winTopHref.substring(winend-1,winend);
      if(wint.indexOf("#")==-1){
      }else{
        winTopHref=winTopHref.substring(0,winend-1);
      }
    }
    else{
...
//			if(strURL.indexOf("?")==-1){
//			 strURL=strURL+"?ntkoieword=ntkoieword";
//     	   }else{
//     		  strURL=strURL+"&ntkoieword=ntkoieword";
//     	   }
//
    ...
    if(browser=="chrome"){
      if(version>="50"){
       ...
       // "","height=759, width=1440, titlebar=no,toolbar =no, menubar=no, scrollbars=no, resizable=yes, location=no, status=no,channelmode=yes"
        window.open(strURL);
      }
...
function CheckNtkoNPAPI()
{
	var mimetype = navigator.mimeTypes["application/ntko-plug"];//这里面的key就是你的插件的type
	if(mimetype)
	{
